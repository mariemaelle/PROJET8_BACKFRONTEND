...
#----------------------------------------------------------------------------
# Affiche la figure avec le client
...
#         # Ajouter un champ pour saisir l'ID du client
#         client_id = st.text_input("Entrez l'ID du client pour visualiser sa position", "")

#         # Si un client est sélectionné, récupérer ses informations via l'API
#         client_value = None
#         if client_id:
...
#--------------------------------------------------------------------
# On va sélectionner le client dans l'onglet 1
...
#     st.markdown("""
#         Le menu déroulant vous permet de sélectionner une des 10 features les plus importantes 
#         afin de visualiser sa distribution et de situer le client parmi tous les autres de la base de donnée.
#     """)
...
#         # Obtenir la situation du client via l'API (si un client a été sélectionné au tout début)
#         if client_id:
#             client_endpoint = f"{api_url}/client/{client_id}"
#             client_response = requests.get(client_endpoint)
#             if client_response.status_code == 200:
#                 client_data = client_response.json()
#                 # Récupérer la valeur de la feature sélectionnée pour le client directement depuis ses données
#                 try:
#                     # La valeur de la feature sélectionnée dans les données du client
#                     client_value = client_data["client_feature_values"][selected_feature]
#                 except KeyError:
#                     st.error(f"La feature sélectionnée '{selected_feature}' n'existe pas dans les données du client.")
#                     client_value = None
...
#                 client_value = None
#         else:
#             st.error("Veuillez sélectionner un ID de client dans l'onglet 1.")
#             client_value = None
...
#         # Ajouter la ligne verticale représentant la situation du client (si disponible)
#         if client_value is not None:
#             ax.axvline(x=client_value, color='black', linestyle='--', label=f"Client ID {client_id}", linewidth=2)
#             ax.legend()

#         st.pyplot(fig)

#         # Récupérer les distributions séparées par la target (0 ou 1)
#         target_0_values = [value for value, t in zip(feature_values, target) if t == 0]
#         target_1_values = [value for value, t in zip(feature_values, target) if t == 1]

#         # Création des deux histogrammes côte à côte
#         fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6), sharey=True)

#         # Histogramme pour la target = 0 (bleu)
#         ax1.hist(target_0_values, bins=30, color='#008BFB', edgecolor='black')
#         ax1.set_title(f"Distribution de {selected_feature} (Target = 0)")
#         ax1.set_xlabel(selected_feature)
#         ax1.set_ylabel("Fréquence")

#         # Ajouter la ligne verticale représentant la situation du client (si disponible)
#         if client_value is not None:
#             ax1.axvline(x=client_value, color='black', linestyle='--', label=f"Client ID {client_id}", linewidth=2)
#             ax1.legend()

#         # Histogramme pour la target = 1 (rose)
#         ax2.hist(target_1_values, bins=30, color='#FF005E', edgecolor='black')
#         ax2.set_title(f"Distribution de {selected_feature} (Target = 1)")
#         ax2.set_xlabel(selected_feature)

#         # Ajouter la ligne verticale représentant la situation du client (si disponible)
#         if client_value is not None:
#             ax2.axvline(x=client_value, color='black', linestyle='--', label=f"Client ID {client_id}", linewidth=2)
#             ax2.legend()

#         # Afficher les deux figures côte à côte dans Streamlit
#         st.pyplot(fig)
